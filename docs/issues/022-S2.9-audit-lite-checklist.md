...
## Solana migration (Audit-lite)

Context
- Checklist an toàn cho Solana/Anchor: account constraints, PDA seeds, lamports/token transfers, authority.

Scope (Solana)
- Permissions: authority-only cho admin instructions.
- Constraints: kiểm tra owners, seeds, signer, rent-exempt.
- Events: đủ cho indexer; logs tối thiểu.
- Asset handling: lamports (native) và SPL Token (token program) an toàn; không có loops unbounded.

Acceptance Criteria
- Checklist PASS; không có blocker.

Tasks
- Rà soát code và cập nhật ghi chú audit-lite theo Solana.
