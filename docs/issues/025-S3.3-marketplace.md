...
- **Primary approach**: Integrate với existing Solana NFT marketplaces (Magic Eden, Tensor, OpenSea Solana)
...
- **User flow**: View owned NFTs → List for sale → Trade on external marketplace
...
|----------|--------------|-------------|---------------------|---------------------|
| **Magic Eden** | ~60% | ✅ REST API + SDK | ✅ On-chain enforced | **High** (Primary) |
| **Tensor** | ~25% | ✅ GraphQL API | ✅ Programmable royalties | **High** (Analytics) |
| **OpenSea** | ~10% | ✅ OpenSea SDK (Solana beta) | ⚠️ Optional (buyer choice) | **Medium** (Fallback) |
| **Solanart** | ~5% | ❌ Manual listing only | ✅ On-chain | **Low** (Reference only) |
...
**Trade-offs**:
- ❌ Less liquidity (external marketplaces have more users)
- ❌ Maintenance burden (security, UX, bugs)
- ✅ Full control (no platform risk)
- ✅ Zero marketplace fees (only network fees)

### Acceptance Criteria

#### External Marketplace
- ✅ Inventory page: "View on Magic Eden" button per NFT
- ✅ Deep link opens correct NFT detail page (verified by mint address)
- ✅ Listed price displayed in inventory (fetched from Magic Eden API)
- ✅ Floor price widget: Shows collection floor (updates every 5 min)
- ✅ Royalty enforcement: 5% creator fee reflected in marketplace listings

#### In-House Marketplace (Optional)
- ✅ List NFT: Creates escrow PDA, deducts listing fee (0.01 SOL)
- ✅ Buy NFT: Transfers SOL → seller, NFT → buyer, 2.5% platform fee
- ✅ Cancel listing: Returns NFT to owner, refunds listing fee
- ✅ Royalty payment: 5% sent to creator wallet automatically

#### Documentation
- ✅ `docs/gamefi/MARKETPLACE.md`: Integration guide, fee breakdown
- ✅ User guide: How to list/buy on external marketplaces
- ✅ Developer docs: API endpoints, SDK usage examples
...
2. Navigate to your profile → "My Items"
3. Find your CatNFT → Click "List for Sale"
4. Set price → Approve transaction
...
### Avoiding Scams:
- ❌ Never approve unknown programs
- ❌ Don't list on untrusted marketplaces
- ✅ Only use official ChocoChoco links
- ✅ Verify mint address matches collection (check Solscan)
...
- ✅ Magic Eden deep links: View NFT, collection page
- ✅ Tensor API integration: Floor price, volume, recent sales
- ✅ Inventory UI: "View on Magic Eden" buttons
- ✅ Listing modal: Price input, fee breakdown, redirect to ME

### Analytics Dashboard
- ✅ Real-time floor price widget (updates every 5 min)
- ✅ Collection stats: Total volume, listed count, avg price
- ✅ Recent sales feed: Last 10 transactions with prices

### In-House Marketplace (POC)
- ✅ Smart contract deployed (Devnet)
- ✅ List/buy/cancel instructions implemented
- ✅ Fee routing: Platform (2.5%), royalty (5%), seller (92.5%)
- 🟡 Frontend UI in progress (80% complete)

### Documentation
- ✅ `docs/gamefi/MARKETPLACE.md`: Complete guide (3000 words)
- ✅ User tutorials: How to list on Magic Eden
- ✅ Fee comparison table: Magic Eden vs Tensor vs OpenSea
...
## Migration Notes: EVM → Solana Marketplaces
...
### Pre-Deploy
- ✅ Verify collection on Magic Eden (submit creator verification)
- ✅ Test API integrations (Magic Eden, Tensor) on devnet
- ✅ Design inventory UI mockups
- ✅ Write user documentation

### Deploy Sequence
1. Submit collection to Magic Eden → Verify creator (1-2 days)
2. Deploy frontend updates → Inventory + marketplace links
3. Test deep links → Ensure NFTs open correctly
4. Optional: Deploy in-house marketplace (Devnet first)
5. Announce to community → Guide users on how to trade

### Post-Deploy Monitoring
- 📊 Daily volume (Magic Eden API)
- 📊 Floor price trends (Tensor API)
- 📊 Listing/sale ratio (how many listings → sales)
- 📊 User feedback (trading UX issues)
